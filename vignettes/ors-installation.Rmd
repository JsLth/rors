...
```{r}
ors$extract$get_extract("Arnsberg")
```
...
After changing any direct changes to the configurations file, the changes have to be saved to the config file using.

```{r}
ors$config$save_config()
```

If you make changes through a provided method or by changing the raw file, this step is not necessary.
...
Docker can be accessed and controlled using `$docker`.
```{r}
ors$docker
```
...
That doesn't seem to work. Looks like I accidentally tried to route between coordinates in Cologne, although I mounted an extract of Arnsberg. This mistake can be fixed by taking down the image:
...
Great. To make sure I don't mess up the coordinates again, I'll use the ORS sampling function `ors_sample`. It's simply a wrapper around `sf::st_sample` that uses the current extract to generate a point sample.
...
get_route_lengths(sample_a, sample_b, profile = "driving-car", how = "matrix")
```
...
Seems to work! However, errors lurk everywhere. If we were to pass a different profile or increase the sample size to 101, we would be faced with new errors. So be sure to take care of your configurations and change them if necessary.
