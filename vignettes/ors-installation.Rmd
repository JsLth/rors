...
If you make changes through a provided method or by changing the raw file, this step is not necessary. However, if you change the raw file, note, that the R6 class can't keep track on its own and needs to be refreshed after saving raw changes by calling `$config <- "refresh"`. The solution of minimal effort and error proneness is making use of the built-in R6 methods since they save changes on their own.
