...
---
# TODO: Rework vignette (preferably with working knitr)
```{r, include = FALSE}
...
* To change the setup settings, you need to bring down the image using `$image_down`
* To change the configurations, you only need to stop the container using `$stop_container`
* To change the OSM extract, graphs must be overwritten by changing `$graph_building` to `change`
* To make sure that graphs are not overwritten when you rebuild the image, assign NA to `graph_building`
* If you change the configurations or settings by assigning values to the parsed files, you need to save the changes using `$save_compose` or `$save_config`, otherwise this step is not necessary