---
title: "Introduction to the ORSRouting functions"
author: Jonas Lieth
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Introduction to the ORSRouting functions}
...
The `ORSRouting` package allows you to easily utilize the power and performance of OpenRouteService to generate routing data. After either setting up a local server or providing a link to an existing server (for details, refer to `vignette("ors-installation")`), you are ready to make requests using one of the package's functions. `ORSRouting` supports three OpenRouteService endpoints, namely:
...
```{r}
nearest_pharmacies <- get_closest_pois(
  sample_a,
  pharmacies,
  number_of_points = 5,
  crs = 4326
...
OpenRouteService itself can only return isochrones as vector data. `ORSRouting` adds a `rasterize` argument to turn these isochrones into a pretty accessibility map that is able to combine multiple isochrone datasets into a single, comprehensible raster image.
