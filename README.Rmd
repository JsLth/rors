...
Some basic functionalities using a running local OpenRouteService instance:

```{r, eval=FALSE}
library(ORSRouting)

# Set up a local OpenRouteService server using Docker
ors <- ors_instance(dir = "~/Documents") %>%
  ors_extract("Rutland") %>%
  ors_config(
    profiles = c("car", "bike-regular", "walking"),
    isochrones.maximum_locations = 20,
    default = c(maximum_snapping_radius = -1, maximum_distance = 1e+07),
    car = c(maximum_snapping_radius = -1, maximum_distance = 1e+07)
  ) %>%
...
`ors_pairwise` returns only distances, durations and geometries - but lots of them (i.e., row-wise):

```{r}
dis <- ors_pairwise(sam[1:10, ], sam[11:20, ], geometry = TRUE, radiuses = -1)
dis
```
